...
private:
  ...
  using MappingPair = std::pair<EthernetAddress, uint32_t>; // ethernet address and ip address

  using NetworkMappings = std::multimap<uint64_t, MappingPair>; // timestamp, mappingpair

  NetworkMappings mappings_ {};

  std::unordered_map<uint32_t, uint64_t> last_arp_request_time_ {}; // ip address, timestamp

  // function
  void addMapping( uint64_t timestamp, const EthernetAddress& eth, uint32_t& ip )
  {
    mappings_.insert( make_pair( timestamp, MappingPair { eth, ip } ) );
  }; // ip can be changed,but eth can't(const reference)

  bool getMapping( uint32_t target_ip, EthernetAddress& eth )
  {
    for ( const auto& m : mappings_ ) {
      if ( m.second.second == target_ip ) {
        eth = m.second.first;
        return true;
      }
    }
  ...
  void remove_expired_mappings()
  {
    ...
    auto it = mappings_.lower_bound( expiration_threshold );
    mappings_.erase( mappings_.begin(), it );
  }
